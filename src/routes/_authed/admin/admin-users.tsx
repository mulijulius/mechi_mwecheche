...
export const Route = createFileRoute('/_authed/admin/users')({
  beforeLoad: async () => {
    ...
    // Suspending/managing player accounts directly is a super admin power.
    // Support and finance manager get a read-only view of users via the
    // /admin/traffic page (active/offline + stats) instead.
    const allowed = profile?.admin_status === 'approved' && profile.admin_role === 'super_admin'
    if (!allowed) throw redirect({ to: '/admin/traffic' })
  },
...
function AdminUsersPage() {
  ...
  async function toggleSuspension(player: PlayerRow) {
    const nextStatus = player.status === 'active' ? 'suspended' : 'active'
    setPlayers((prev) =>
      prev.map((p) => (p.id === player.id ? { ...p, status: nextStatus } : p)),
    )
    await supabase.from('profiles').update({ status: nextStatus }).eq('id', player.id)
  }
              ...
              {filtered.map((player) => (
                <li key={player.id} className="flex items-center justify-between px-5 py-3">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <Button variant="ghost" size="icon" title="Toggle suspension" onClick={() => toggleSuspension(player)}>
                      {player.status === 'active' ? (
