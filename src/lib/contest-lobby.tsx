/**
 * src/components/ludo/contest-lobby.tsx
 *
 * Real-time list of open Ludo contests. Subscribes to Supabase realtime via
 * LudoProvider and shows a join flow with wallet/trial verification.
 * Mirrors src/components/checkers/contest-lobby.tsx — the main difference is
 * showing a "seated/max" badge instead of a fixed 1v1 indicator, since a
 * Ludo contest can need 2, 3, or 4 players before it locks.
 */
...
import { useLudo, type OpenContest, type JoinResult } from '#/lib/ludo-context'
...
const JOIN_ERROR_MSG: Record<JoinResult, string> = {
  ...
  already_full:         'This table just filled up. Refresh to see updated tables.',
  not_found:            'Contest no longer exists.',
  trial_limit_reached:  'You have used all 3 free trials today. Deposit to keep playing.',
...
export function ContestLobby() {
  ...
  const { openContests, isLobbyLoading, joinContest, trialRemaining } = useLudo()
  ...
  async function handleJoin(contest: OpenContest) {
    ...
    if (result === 'ok') {
      navigate({ to: '/dashboard/ludo/$contestId', params: { contestId: contest.id } })
    } else {
  ...
  function canJoin(contest: OpenContest): boolean {
    ...
    if (contest.seated_players >= contest.max_players) return false
  ...
  if (openContests.length === 0) {
    return (
      <div className="flex h-40 flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-arena-border text-center">
        ...
        <p className="text-xs text-arena-text-dim opacity-60">Host one and wait for challengers!</p>
      </div>
    )
  }
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="default" className="gap-1 text-[10px]">
                  ...
                  {contest.seated_players}/{contest.max_players}
                </Badge>
              </div>
